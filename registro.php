    ...
    <main>
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['enviar'])) {
          list($datos, $errores) = validarDatos();

          if (empty($errores)) {
            if (insertar_usuario($datos['nombre_usuario'], $datos['apellidos'], $datos['DNI'], $datos['email'], $datos['clave'], $datos['foto'], $datos['rol'])) {
              echo '<h3 class="confirmacion">Usuario registrado correctamente.</h3>';
              $descripcion = "El usuario con DNI '" . htmlspecialchars($_SESSION['DNI']) . "' se ha registrado exitosamente.";
              registrar_evento_log("registro", $descripcion);
            } else {
              echo '<h3 class="aseguro">Error al insertar el usuario.</h3>';
            }
          } else {
            $descripcion = "Un usuario ha intentado registrase sin éxito.";
            registrar_evento_log("registroFail", $descripcion);
